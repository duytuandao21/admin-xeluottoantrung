...
export default function CamNhanKHPage() {
  return <CrudPage title="Cảm nhận khách hàng" data={mockTestimonials as unknown as Record<string, unknown>[]} columns={[
    ...
    { key: 'rating', label: 'Đánh giá', render: (item) => <div className="flex gap-0.5">{Array.from({ length: Number(item.rating) }, (_, i) => <Star key={i} className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />)}</div> },
